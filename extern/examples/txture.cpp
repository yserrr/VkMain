...
void Renderer::pipelineSetup()
{
  ...
  UboPool_ = std::make_unique<Buffer>(*allocator_,
                                        BufferType::UNIFORM,
                                        camSize,
  ...
  fragshader->setShader("/home/ljh/Desktop/MyProject-master/source/back/gpu_vk/render_ps/exp_shader/lightFrag.frag",
  ...
  PipelinePoolCreateInfo plpCi{};
  plpCi.baseLayout            = &layout_h;
  plpCi.renderPass            = VK_NULL_HANDLE;
  plpCi.device                = device_h;
  plpCi.descriptorLayoutCount = 1;
  pipelinePool_               = std::make_unique<PipelinePool>(plpCi);
  PipelineCreatePass pass{};
  VkFormat colorformat  = (swapchain_->getFormat());
  pass.colorFormats     = &colorformat;
  pass.depthFormat      = VK_FORMAT_D32_SFLOAT;
  pass.colorFormatCount = 1;
  pass.cullMode         = VK_CULL_MODE_NONE;
  pass.fragShaderModule = frag;
  pass.vertShaderModule = vert;
  //pipeline_h =  pipelinePool_->createPipeline(pass);
}

void Renderer::transferStage()
{
  ...
  for (uint32_t i = 0; i < 1; i++)
  {
    ...
    bufferCopyRegion.bufferOffset                    = imgBlock.offset + offset;
