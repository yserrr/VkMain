...
///  default setting
///  current based on shader reflection system
///  -> all interface centered on shader and shader graph
///  it's very important
///  descriptor not determined by resource allocate
///  descriptor determine on always shader
///  -> shader changed,
///     pipeline should rebuild
  ...
  DescriptorSet sets(device_h);
  sets.createPool(cam_dsl, gpu::descriptor_usage::DYNAMIC_UBO);
  sets.createPool(bindless_dsl, gpu::descriptor_usage::TEXTURE_BINDLESS);
  sets.createPool(light_dsl, gpu::descriptor_usage::UBO);

  gpu::descriptor_set_state camSetState      = sets.allocate(gpu::descriptor_usage::DYNAMIC_UBO);
  gpu::descriptor_set_state bindlessSetStaet = sets.allocate(gpu::descriptor_usage::TEXTURE_BINDLESS);
  gpu::descriptor_set_state lightSetStae     = sets.allocate(gpu::descriptor_usage::UBO);
  gpu::descriptor_set_state materialState    = sets.allocate(gpu::descriptor_usage::UBO);
  MeshDesc desc;
  MeshManager mesh;
